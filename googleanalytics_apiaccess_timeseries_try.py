...
def get_results(service, profile_id):
	...
	start = proper_start_date(15)
	end = str(date.today())
	# return query.execute()
	return service.data().ga().get(ids='ga:%s' % (profile_id), start_date="%s" % start,end_date=end,metrics='ga:pageviews',dimensions='ga:date',sort='ga:date',filters='ga:pagePath==%s' % (sys.argv[2])).execute()
